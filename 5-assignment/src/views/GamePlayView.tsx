...
export default function GamePlayView() {
  ...
  const winnerIndex = derived ? ((): number | undefined => {
    for (let i = 0; i < derived.scores.length; i++) if (derived.scores[i]! >= derived.targetScore) return i
    return undefined
  })() : undefined
  ...
  if (serverGame.status === 'WAITING') {
    return (
      <div>
        ...
        <button onClick={() => dispatch(startGame({ gameId }))} disabled={serverGame.players.length < 2}>
          Start game
        </button>
      </div>
    )
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
        <div>
          ...
          <div onClick={() => (isMyTurn ? dispatch(drawCard({ gameId })) : null)} style={{ cursor: isMyTurn ? 'pointer' : 'not-allowed' }}>
            <Pile type="DRAW" />
          </div>
          <div style={{ opacity: 0.7, marginTop: 6 }}>{isMyTurn ? 'Click to draw' : 'Wait your turn'}</div>
        </div>
