...
import { loadGame, startGame, drawCard, playCard } from '@/features/game/gameSlice'
...
/**
 * Prompt the user to choose a colour for wild cards.  This is invoked
 * client‑side whenever a player plays a wild or wild draw four.  It
 * mirrors the behaviour from the original Vite app.
 */
function promptColor(): Color | undefined {
  const v = window.prompt('Choose color: RED, GREEN, BLUE, YELLOW')?.trim().toUpperCase()
  if (!v) return undefined
  if (v === 'RED' || v === 'GREEN' || v === 'BLUE' || v === 'YELLOW') return v as Color
  return undefined
}

/**
 * Play page.  Displays the current state of the game and allows the
 * authenticated user to draw and play cards.  The server‑side props
 * ensure that unauthenticated users are redirected before any of this
...
export default function PlayPage() {
  ...
  // Load the latest game state whenever the gameId changes.  This
  // subscription is client‑side only; the initial redirect happens in
  // getServerSideProps.
  useEffect(() => {
    if (gameId) dispatch(loadGame({ gameId }))
  }, [dispatch, gameId])

  // Derive a local representation of the game from the server memento.
  ...
  const winnerIndex: number | undefined = useMemo(() => {
  ...
  // Navigate to the game over page when a winner has been determined.
  useEffect(() => {
    if (winnerIndex !== undefined) router.push('/game-over')
  }, [winnerIndex, router])
  ...
  if (!gameId) {
    return (
      <div>
        ...
        <button onClick={() => router.push('/')}>Go to Setup</button>
      </div>
    )
  }
  ...
  if (serverGame.status === 'WAITING') {
    return (
      <div>
        ...
        <p>
          Players: {serverGame.players.map((p) => p.username).join(', ')} ({serverGame.players.length}/
          {serverGame.amountOfPlayers})
        </p>
        <button
  ...
  return (
    <div>
      ...
      <p>
        Turn: <b>{players[playerInTurn ?? 0] ?? '—'}</b> {isMyTurn ? '(you)' : ''}
      </p>
...
// Redirect to the login page on the server if there is no auth token.  This
// guards the play route against unauthenticated access before any
