...
function ensurePlayers(players: string[]): void {
  if (players.length < MIN_AMOUNT_OF_PLAYERS || players.length > MAX_AMOUNT_OF_PLAYERS) {
    throw new Error(
...
export function player(state: RoundState, index: number): string {
  if (index < 0 || index >= state.players.length) throw new Error("Player index out of bounds");
  return state.players[index]!;
}

export function playerHand(state: RoundState, playerIndex: number): ReadonlyArray<Card> {
...
export function canPlay(state: RoundState, cardIndex: number): boolean {
  ...
  const hasColorCardWithCurrentColor = hand.some(
    (c) => c.type !== "WILD" && c.type !== "WILD DRAW" && "color" in c && (c as any).color === state.currentColor
  );

  // Standard house-rule used in the provided model: cannot play Wild Draw if you can follow color
  if (currentCard.type === "WILD DRAW" && hasColorCardWithCurrentColor) return false;

  if (currentCard.type === "WILD" || currentCard.type === "WILD DRAW") return true;

  if ("color" in currentCard && (currentCard as any).color === state.currentColor) return true;

  if (currentCard.type === topCard.type && currentCard.type !== "NUMBERED") return true;
  ...
  switch (topCard.type) {
    ...
    case "REVERSE": {
      ...
      const newDir: Direction =
        nextState.currentDirection === "counterclockwise" ? "clockwise" : "counterclockwise";
...
export function createRound(args: {
  ...
  if (!first.card) throw new Error("Not enough cards in the deck to start the round");
  drawPile = first.deck;
  ...
  let state: RoundState = {
    ...
    discardPile: discard,
    hands,
    currentColor: ("color" in first.card ? (first.card as any).color : "RED") as Color,
...
export function fromMemento(memento: any, shuffler: Shuffler<Card>): RoundState {
  ...
  if (memento.currentColor === undefined || !colors.includes(memento.currentColor)) {
  ...
  if (topCard.type !== "WILD" && topCard.type !== "WILD DRAW" && (topCard as any).color !== memento.currentColor) {
...
export function catchUnoFailure(
  ...
  if ((state.hands[accused]?.length ?? 0) !== 1) return { state, success: false };
  if (state.playerInTurn === accused) return { state, success: false };

  const n = state.players.length;
  const nextPlayerIndex =
    state.currentDirection === "clockwise" ? (accused + 1) % n : (accused - 1 + n) % n;
  ...
  const topCard = state.discardPile[0];
  const isLastCardForDrawing = topCard?.type === "DRAW" || topCard?.type === "WILD DRAW";
...
export function draw(state: RoundState, shuffler: Shuffler<Card>): { state: RoundState; handSize: number } {
  if (hasEnded(state)) throw new Error("Round is over");
  const p = state.playerInTurn!;
  let s = { ...state, unoCalledBy: [] as number[] };
...
export function play(
  ...
  const p = state.playerInTurn!;
  const hand = state.hands[p] ?? [];
  if (cardIndex < 0 || cardIndex >= hand.length) throw new Error("Illegal move");
