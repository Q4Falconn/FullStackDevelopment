...
describe("Initial deck", () => {
  ...
  it("contains 19 numbered blue cards", () => {
    expect(
      initialDeck.filter(is({ type: "NUMBERED", color: "BLUE" })).size
    ).toEqual(19);
  });
  it("contains 19 numbered green cards", () => {
    expect(
      initialDeck.filter(is({ type: "NUMBERED", color: "GREEN" })).size
    ).toEqual(19);
  });
  it("contains 19 numbered red cards", () => {
    expect(
      initialDeck.filter(is({ type: "NUMBERED", color: "RED" })).size
    ).toEqual(19);
  });
  it("contains 19 numbered yellow cards", () => {
    expect(
      initialDeck.filter(is({ type: "NUMBERED", color: "YELLOW" })).size
    ).toEqual(19);
  });
...
describe("fromMemento", () => {
  ...
  describe("from invalid Memento", () => {
    ...
    it("throws on missing number on numbered type", () => {
      expect(() =>
        createDeckFromMemento([{ type: "NUMBERED", color: "BLUE" }])
      ).toThrowError();
    });
    it("throws on missing color on numbered type", () => {
      expect(() =>
        createDeckFromMemento([{ type: "NUMBERED", number: 7 }])
      ).toThrowError();
    });
